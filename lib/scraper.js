...
export async function scrapeFollowersFromPage(page, targetAccount, options = {}) {
    ...
    try {
        ...
        const followersLink = page.locator(`a[href="/${target}/followers/"]`).first();
        if (await followersLink.count() === 0) {
            // Intentar alternativa en español
            const followersAlt = page.locator('a[href*="/followers"]').first();
            if (await followersAlt.count() > 0) {
                await followersAlt.click();
            } else {
        ...
        const maxScrollAttempts = 60; // Más paciencia para keywords
        let noNewLeadsCount = 0;

        onLog(`[SCRAPER] 🔄 Iniciando recolección de handles (Objetivo: ${maxLeads})...`);
        ...
        while (leads.length < maxLeads && scrollAttempts < maxScrollAttempts) {
        ...
        for (let i = 0; i < leads.length; i++) {
            ...
            try {
                ...
                if (existsRes.rows.length > 0 && existsRes.rows[0].status !== 'pendiente') {
            ...
            try {
                ...
                onLog(`[SCRAPER] ✅ @${lead.username} — "${lead.biography.substring(0, 60)}..."`);
