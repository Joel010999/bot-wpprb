 ...
 * Uses authenticated bot sessions (fleet.js) to extract followers/following from Instagram
 * Includes human-like delays for anti-detection (optimizado para Jujuy)
 */

// ── Delays Humanos Anti-Detección ──
const SCROLL_DELAY = () => Math.floor(Math.random() * 3000) + 2000;
const PROFILE_VISIT_DELAY = () => Math.floor(Math.random() * 7000) + 5000;
const BATCH_PAUSE = () => Math.floor(Math.random() * 30000) + 30000;
const AFTER_CLICK_DELAY = () => Math.floor(Math.random() * 3000) + 2000;
...
export async function scrapeFollowersFromPage(page, targetAccount, options = {}) {
    ...
    const rawLimit = maxLeads * 3;
    const nicheKeywords = options.nicheKeywords || [];
    const searchKeyword = options.searchKeyword || "";
    const scrapeType = options.scrapeType || "followers"; // NUEVO: Extraemos la opción (followers o following)
    ...
    const typeLabel = scrapeType === "following" ? "seguidos" : "seguidores"; // Etiqueta para los logs
    ...
    try {
        ...
        onLog(`[SCRAPER] 📋 Abriendo lista de ${typeLabel} de @${target}...`);

        // NUEVO: La magia del selector dinámico según la opción que mande la UI
        let targetLink;
        if (scrapeType === "following") {
            targetLink = page.locator([
                `a[href="/${target}/following/"]`,
                'a[href*="/following"]',
                'button:has-text("Seguidos")',
        ...
        if (await targetLink.count() > 0) {
            ...
            onLog(`[SCRAPER] ❌ No se encontró el link de ${typeLabel}. Cuenta posiblemente privada o sin ${typeLabel}.`);
            return leads;
        }

        await page.waitForTimeout(AFTER_CLICK_DELAY());

        onLog(`[SCRAPER] 📊 Modal de ${typeLabel} abierto. Comenzando extracción...`);
...
export async function scrapeFollowers(targetUrl, options = {}) {
    ...
    try {
        ...
        const leads = await scrapeFollowersFromPage(page, target, {
            ...
            scrapeType: options.scrapeType || "followers",
