...
async function processRun(ai, threadId, assistantId, additionalConfig = {}) {
    ...
    // 3. Bucle de espera usando los IDs fijos
    while (run.status === "in_progress" || run.status === "queued") {
        await new Promise((resolve) => setTimeout(resolve, 2000));

        // RE-RETRIEVE: Forzamos el uso de las constantes inmutables
        run = await ai.beta.threads.runs.retrieve(targetThreadId, runId);
    }

    if (run.status === "requires_action") {
        console.error(`[OpenAI] Run requiere acción. Cancelando.`);
        await ai.beta.threads.runs.cancel(targetThreadId, runId);
