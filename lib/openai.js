...
// Inicializa o recupera el Asistente (DB → crear nuevo, NUNCA env var)
...
// ── LÓGICA DE RUNS BLINDADA (VERSION FINAL - JOEL FIX) ──
async function processRun(ai, threadId, assistantId, additionalConfig = {}) {
    // 1. Sanitización de IDs con constantes inmutables
    const targetThreadId = String(threadId || "").trim();
    const targetAssistantId = String(assistantId || "").trim();

    if (!targetThreadId || targetThreadId === "undefined" || !targetAssistantId || targetAssistantId === "undefined") {
    ...
    console.log(`[DEBUG AI] Iniciando Run — Thread: ${targetThreadId} | Assistant: ${targetAssistantId}`);

    // 2. Crear el Run
    let run = await ai.beta.threads.runs.create(targetThreadId, runArgs);
    const runId = run.id;

    // 3. Bucle de espera usando los IDs que el propio objeto Run confirma
    while (run.status === "in_progress" || run.status === "queued") {
        await new Promise((resolve) => setTimeout(resolve, 2000));

        // RE-RETRIEVE: Forzamos el uso de los IDs directos del objeto Run para que no haya undefined
        run = await ai.beta.threads.runs.retrieve(run.thread_id, run.id);
    }

    if (run.status === "requires_action") {
        console.error(`[OpenAI] Run requiere acción. Cancelando.`);
        await ai.beta.threads.runs.cancel(run.thread_id, run.id);
    ...
    if (lastMsg.role === "assistant" && lastMsg.content[0].type === "text") {
        ...
        // REFUERZO DE LIMPIEZA JOEL (Estricto)
        text = text.replace(/[¿—–]/g, "");
        text = text.replace(/^-+/, "");
