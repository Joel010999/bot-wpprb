...
async function processRun(ai, threadId, assistantId, additionalConfig = {}) {
    // BLINDAJE: Validar IDs críticos antes de cualquier llamada a la API
    if (!threadId || !assistantId) {
        throw new Error(`Faltan IDs críticos — threadId: ${threadId}, assistantId: ${assistantId}`);
    }

    // Sanitizar threadId por si llega con espacios o caracteres raros
    ...
    console.log(`[DEBUG] Usando Thread: ${threadId} | Assistant: ${assistantId}`);
