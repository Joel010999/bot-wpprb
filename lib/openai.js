...
// Lógica universal para Runs (VERSION CORREGIDA - SIN UNDEFINED)
async function processRun(ai, threadId, assistantId, additionalConfig = {}) {
    // 1. Sanitización absoluta de IDs
    const tId = String(threadId || "").trim();
    const aId = String(assistantId || "").trim();

    if (!tId || tId === "undefined" || !aId || aId === "undefined") {
        throw new Error(`Faltan IDs críticos para OpenAI — threadId: ${tId}, assistantId: ${aId}`);
    }
    ...
    // 2. Crear el Run usando las constantes sanitizadas
    let run = await ai.beta.threads.runs.create(tId, runArgs);

    // 3. Bucle de espera con IDs blindados
    while (run.status === "in_progress" || run.status === "queued") {
        await new Promise((resolve) => setTimeout(resolve, 1500));

        // RE-RETRIEVE: Usamos tId (constante) para evitar el error de segmento undefined
    ...
    if (lastMsg.role === "assistant" && lastMsg.content[0].type === "text") {
        ...
        // REFUERZO DE LIMPIEZA JOEL: Sin signos de apertura ni guiones
        text = text.replace(/[¿—–]/g, "");
        text = text.replace(/^-+/, "");
        text = text.replace(/\s+/g, " ");

        // Sin punto final al terminar
