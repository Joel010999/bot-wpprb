...
// closeModals → Like (3 reintentos + force) → Espera → DM
...
export async function sendAndVerifyDM(page, targetHandle, messageOrData, onLog = () => { }) {
    ...
    // ── PASO 0: ENGAGEMENT PREVIO (Like) antes de tocar el chat ──
    ...
    // CAPTURA DE DATA: Tomamos el mensaje de la IA o el fallback
    const aiMessage = fallbackMessage;

    // Función de limpieza de estilo estricta (Elimina signos de apertura y guiones, permite comas)
    const cleanStyle = (txt) => txt.replace(/[¿—\-]/g, "").trim();

    // FRAGMENTACIÓN HUMANA: Dividir por puntos, exclamaciones o saltos de línea
    const fragments = aiMessage.split(/[.!]+|\n+/).map(f => cleanStyle(f)).filter(f => f.length > 0);
    ...
    try {
        ...
        await messageInput.click({ force: true, timeout: 15000 });
    ...
    for (let i = 0; i < fragments.length; i++) {
        ...
        // Presión de Enter inmediata
        await page.keyboard.press("Enter");

        // REGLA DE ENVÍO: Esperar entre 1.5 y 3 segundos antes del siguiente fragmento
        if (i < fragments.length - 1) {
            const humanPause = Math.floor(Math.random() * 1500) + 1500;
