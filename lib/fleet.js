...
const TYPING_DELAY = () => Math.floor(Math.random() * 41) + 30;       // 30-70ms por tecla
const BETWEEN_WORDS_PAUSE = () => Math.floor(Math.random() * 301) + 200; // 200-500ms pausas entre palabras
...
export async function sendAndVerifyDM(page, targetHandle, messageOrData, onLog = () => { }) {
    ...
    try {
        ...
        onLog(`[${ts()}] ⏳ Input no detectado tras click (10s). Intentando reload y re-click en botón de mensaje...`);
    ...
    for (let i = 0; i < fragments.length; i++) {
        ...
        // Pausa de 1 segundo exacta requerida (Estilo Lorenzo)
