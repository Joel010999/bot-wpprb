...
const TYPING_DELAY = () => Math.floor(Math.random() * 200) + 80;       // 80-280ms por tecla
const BETWEEN_WORDS_PAUSE = () => Math.floor(Math.random() * 2000) + 500; // pausas entre frases
...
function getSessionPath(username) {
    ...
    return join(SESSIONS_DIR, `${username}.json`);
}
...
export async function saveSession(username, context) {
    try {
        ...
        console.log(`[Fleet] Sesión guardada para @${username}`);
        return true;
    } catch (err) {
...
export async function createBotSession(account, onLog = console.log) {
    ...
    const browser = await chromium.launch({
        headless: false, // ⚠️ FORZADO SIEMPRE VISIBLE — NO CAMBIAR
        args: [
    ...
    // Intentar cargar sesión previa (cookies, localStorage)
    const savedState = loadSessionState(account.username);
    ...
    // Si hay sesión guardada, inyectarla en el contexto
    if (savedState) {
        contextOptions.storageState = savedState;
        onLog(`[SISTEMA] 📂 Sesión previa cargada para @${account.username}`);
    } else {
...
export async function humanSocialEngagement(page, targetHandle, onLog = () => { }) {
    ...
    // Asegurar 15 segundos totales de interacción
    ...
    onLog(`[${ts()}] ✅ Protocolo de Engagement de 15s completado.`);
...
export async function sendAndVerifyDM(page, targetHandle, messageOrData, onLog = () => { }) {
    ...
    // Pausa humana de 15s post-engagement antes de buscar el botón de mensaje (Solicitud User)
    onLog(`[${ts()}] ⏱️ Pausa humana 15s post-engagement antes de ir al Mensaje...`);
    ...
    try {
        ...
        onLog(`[${ts()}] ⏳ Input no detectado tras click. Intentando navegación directa...`);
    ...
    if (!inputFound) {
        onLog(`[${ts()}] ❌ Fallo crítico: Input de chat no encontrado tampoco en URL directa.`);
    ...
    let lastLeadMessage = "";
    let hasPreviousMessages = chatHistory.length > 0;
    ...
    for (let i = 0; i < fragments.length; i++) {
        ...
        for (const char of msgToSend) {
            await page.keyboard.type(char, { delay: TYPING_DELAY() });
            if (Math.random() < 0.1) await page.waitForTimeout(BETWEEN_WORDS_PAUSE());
        }

        // Pausa de "escritura/pensamiento" entre 1.5 a 3 segundos antes de enviar
        const typingPause = Math.floor(Math.random() * 1500) + 1500;
        onLog(`[${ts()}] ⏳ Pausa de tipeo (${typingPause}ms)`);
        await page.waitForTimeout(typingPause);

        await page.keyboard.press("Enter");
        
        // Pequeña pausa adicional si NO es el último mensaje
...
export async function scrapeChatHistory(page, onLog = null) {
    ...
    const UI_TEXT_BLACK_LIST = [
        "Following", "Siguiendo", "Seguindo", "Message", "Enviar mensaje", "Ver perfil", "Ver perfil",
        "Requested", "Solicitado", "Solicitado", "Follow back", "Seguir también", "Seguir de volta",
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (!cleanTxt || cleanTxt.length < 1) continue;
        // Filtro de ruido manual (evitar capturar UI como mensajes)
        const isUI = ["Sincronizar", "Following", "Siguiendo", "Message", "Mensaje", "Requested", "Visto", "Seen"].some(noise => cleanTxt.includes(noise));
        if (isUI) continue;
