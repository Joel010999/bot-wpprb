...
export async function saveSession(username, context) {
    try {
        ...
        await db.execute({
            sql: `INSERT INTO bot_sessions (username, storage_state, updated_at)
                  VALUES (?, ?, CURRENT_TIMESTAMP)
                  ON CONFLICT(username) DO UPDATE SET
                  storage_state = EXCLUDED.storage_state,
                  updated_at = CURRENT_TIMESTAMP`,
            args: [username, stateStr]
        });
...
export async function hasSession(username) {
    try {
        ...
        const res = await db.execute({
            sql: "SELECT 1 FROM bot_sessions WHERE username = ?",
            args: [username]
        });
...
async function loadSessionState(username) {
    try {
        ...
        const db = await getDb();
        const res = await db.execute({
            sql: "SELECT storage_state FROM bot_sessions WHERE username = ?",
            args: [username]
        });
        
        if (res.rows.length > 0 && res.rows[0].storage_state) {
            return JSON.parse(res.rows[0].storage_state);
        }
        return null;
    } catch (err) {
