...
// Enviar y Verificar DM (MODIFICADO: ANTI TRAMPA DE PERFIL Y CLICK JS)
export async function sendAndVerifyDM(page, targetHandle, messageOrData, onLog = () => { }) {
    ...
    // CLICKEO AGRESIVO CON JAVASCRIPT PURO
    let clicked = false;
    try {
        clicked = await page.evaluate(() => {
            ...
            const btn = elements.find(el => {
                ...
                return text === 'message' || text === 'mensaje' || text === 'enviar mensaje';
            });
            if (btn) {
                btn.click();
                return true;
        ...
        if (clicked) {
            onLog(`[${ts()}] ✅ Botón de Mensaje clickeado (Forzado JS). Esperando navegación...`);
        } else {
            onLog(`[${ts()}] ⚠️ Botón de Mensaje no encontrado en el DOM.`);
        }
    } catch (e) {
        onLog(`[${ts()}] ⚠️ Error inyectando click JS.`);
    }

    await page.waitForTimeout(4000);

    // VERIFICACIÓN CRÍTICA DE "TRAMPA DE PERFIL"
    if (!page.url().includes('/direct/')) {
        onLog(`[${ts()}] 🚨 ALERTA: El click no nos sacó del perfil. Forzando extracción de link...`);
        try {
            ...
            if (href) {
                onLog(`[${ts()}] ✅ Link directo encontrado. Navegando forzado a la URL...`);
                await page.goto(`https://www.instagram.com${href}`, { waitUntil: "domcontentloaded", timeout: 30000 });
            } else {
                onLog(`[${ts()}] ❌ No se encontro link a DMs en este perfil. Puede estar restringido.`);
                return { sent: false, verified: false, error: "Perfil restringe envio de mensajes" };
            }
        } catch (e) { }
        await page.waitForTimeout(4000);
    }

    // SELECTOR UNIVERSAL Y DEFINITIVO (Usamos el ultimo de la pagina para evitar barras de busqueda)
    const INPUT_SELECTORS = 'div[role="textbox"][contenteditable="true"], textarea';
    let messageInput = page.locator(INPUT_SELECTORS).last();
    ...
    try {
        ...
        onLog(`[${ts()}] ⚠️ Falló carga inicial del input. Rompiendo popups invisibles...`);

        try {
            await page.keyboard.press("Escape");
            const dim = await page.evaluate(() => ({ w: window.innerWidth, h: window.innerHeight }));
            await page.mouse.click(dim.w / 2, dim.h - 50).catch(() => { });
        } catch (err) { }
        ...
        try {
            ...
            onLog(`[${ts()}] ⚠️ Input sigue sin aparecer. Recargando la vista de chat...`);
            await page.reload({ waitUntil: "domcontentloaded", timeout: 30000 });
            await page.waitForTimeout(5000);
            await closeModals(page, onLog, ts);

            messageInput = page.locator(INPUT_SELECTORS).last();
            try {
    ...
    if (!inputFound) {
        onLog(`[${ts()}] ❌ Fallo crítico definitivo: Input de chat no encontrado tras comprobacion de URL.`);
        await page.screenshot({ path: "debug_input_fail.png" }).catch(() => { });
        return { sent: false, verified: false, error: "Campo de texto no encontrado" };
    }

    onLog(`[${ts()}] 📨 Chat cargado y input detectado con exito.`);
    ...
    const fragments = [
        "Hola, como estas?",
        "Te escribo porque me dedico a desarrollar soluciones digitales para negocios, como páginas web, sistemas y automatizaciones",
        "Estuve viendo tu perfil y creo que hay algunas oportunidades interesantes para potenciar tu presencia online",
        "Te gustaria que te muestre algunas ideas aplicadas especificamente a tu negocio?",
    ];
