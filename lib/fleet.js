...
export async function saveSession(username, context) {
    try {
        ...
        const sqlQuery = db.isPostgres
            ? `UPDATE bot_accounts SET session_data = ?, last_active = NOW() WHERE username = ?`
            : `UPDATE bot_accounts SET session_data = ?, last_active = CURRENT_TIMESTAMP WHERE username = ?`;

        await db.execute({
            sql: sqlQuery,
            args: [stateStr, username]
        });
