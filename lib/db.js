...
export function getDb() {
  if (!db) {
    ...
    if (dbUrl && dbUrl.startsWith("postgres://")) {
      ...
      db = {
        isPostgres: true,
        execute: async (queryOrSql, params = []) => {
          let sql = typeof queryOrSql === "string" ? queryOrSql : queryOrSql.sql;
          let args = typeof queryOrSql === "string" ? params : (queryOrSql.args || []);
      ...
      db = {
        isPostgres: false,
        execute: async (queryOrSql, params = []) => {
          if (typeof queryOrSql === "string" && params && params.length > 0) {
             const res = await libDb.execute({ sql: queryOrSql, args: params });
             return { rows: res.rows };
          }
